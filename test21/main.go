...
func main() {
	...
	// 画像をBase64に埋め込み、相対パスを絶対パスに変換
	baseURL, _ := url.Parse(urlStr)
	var buf bytes.Buffer
	embedImagesAndScripts(&buf, doc, baseURL)

	// 埋め込み済みHTMLをデータベースに直接保存
	saveToDatabaseWithGORM(buf.Bytes())
}

// 画像をBase64に変換して埋め込み、相対パスを絶対パスに変換、<script>タグを保持
func embedImagesAndScripts(w io.Writer, node *html.Node, baseURL *url.URL) {
	if node.Type == html.ElementNode {
		...
		if node.Data == "img" {
			for i, attr := range node.Attr {
				if attr.Key == "src" {
					imgData, err := downloadAndEncodeImage(attr.Val)
					if err != nil {
						fmt.Println("Failed to download image:", err)
						return
					}
					node.Attr[i].Val = imgData // 画像URLをBase64データに置き換え
					break
				}
			}
		}
	...
	for c := node.FirstChild; c != nil; c = c.NextSibling {
		embedImagesAndScripts(w, c, baseURL)
	}
